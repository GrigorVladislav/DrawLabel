...
class MyViewController : UIViewController {
    ...
    override func loadView() {
        ...
        label1.frame = CGRect(x: 0, y: 300, width: screenWidth-400, height: 50)
        label2.frame = CGRect(x: 0, y: 350, width: screenWidth-400, height: 50)
        firstLabel()
        secondLabel()
    ...
    @IBAction func tapLabel(gesture: UITapGestureRecognizer) {
        if gesture.didTapAttributedTextInLabel(label: label2, inRange: link) {
            if label1.backgroundColor == UIColor.magenta {
